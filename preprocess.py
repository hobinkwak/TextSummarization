...
def prepro_like_morphlized(data, method='Okt'):
    if method == 'Okt':
        morph_analyzer = Okt()
    if method == 'Komoran':
...
def load_vocabulary(path, vocab_path, tokenize_as_morph=False, method='Okt'):
    # 사전을 담을 배열 준비한다.
    vocabulary_list = []
    ...
    if not os.path.exists(vocab_path):
        ...
        if (os.path.exists(path)):
            ...
            if tokenize_as_morph:  # 형태소에 따른 토크나이져 처리
                print(method)
                question = prepro_like_morphlized(question, method)
                answer = prepro_like_morphlized(answer, method)
            data = []
            # 질문과 답변을 extend을
            # 통해서 구조가 없는 배열로 만든다.
